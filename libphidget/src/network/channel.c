...
PhidgetReturnCode
openServerChannel(uint64_t pid, Phidget_ChannelClass chclass, int chidx, PhidgetNetConnHandle nc,
  PhidgetChannelHandle *openChannel, uint16_t reqseq) {
	...
	if (res != EPHIDGET_OK) {
		netlogerr("failed to add network connection '%"PRIphid"' to device '%"PRIphid"': %s", nc, device, mos_notice_name(res));
		goto bad;
	}
	...
	if (!PhidgetCKFlags(channel, PHIDGET_OPEN_FLAG)) {
		...
		if (res != EPHIDGET_OK) {
			if (res != EPHIDGET_BUSY) // do not spam the log file
				netlogerr("attachChannel(%"PRIphid") failed '%s' for '%"PRIphid"'", channel, mos_notice_name(res), nc);
		...
		if (res != EPHIDGET_OK) {
			netlogerr("dispatchChannelSetStatus(%"PRIphid") failed '%s' for '%"PRIphid"'", channel, mos_notice_name(res), nc);
			removeChannelNetworkConnection(channel, nc, &cnt);
			goto bad;
		}
	}
...
channelDeliverBridgePacket(PhidgetChannelHandle channel, BridgePacket *bp, PhidgetNetConnHandle nc,
  int forward) {
	...
	netlogverbose("bridgeInput(%"PRIphid") == %s", channel, mos_notice_name(res));
	if (res != EPHIDGET_OK)
		return (res);
