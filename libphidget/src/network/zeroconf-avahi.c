  ...
  AvahiLookupResultFlags flags, void *ctx) {	
	...
	case AVAHI_RESOLVER_FOUND: 
		if (maddr) {
			...
			maddr->s4.sin_family = AF_INET;
			maddr->s4.sin_addr.s_addr = address->data.ipv4.address;
			maddr->s4.sin_port = port;
		}
		if (_kv) {
			if (txt != NULL) {
				newkv(&kv);
...
Zeroconf_lookup(ZeroconfListenerHandle handle, int interface, Zeroconf_Protocol proto, const char *name,
  const char *host, const char *type, const char *domain, Zeroconf_Protocol reqproto, mos_sockaddr_t *addr,
  kv_t **kv) {
