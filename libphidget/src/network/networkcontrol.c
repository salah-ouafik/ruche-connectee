...
#define nclogverbose(...) PhidgetLog_loge(NULL, 0, NULL, NETCTLS, PHIDGET_LOG_VERBOSE, __VA_ARGS__)
#else
...
runNetworkControlEntry(void *arg) {
	...
	ncloginfo("started network control entry for %s", nce->name);
	while (nce->flags & NCE_ENABLED) {
		...
		if ((nce->flags & NCE_MDNS) && nce->handle) {
			...
			res = Zeroconf_lookup(nce->handle, nce->interface, ZCP_IPv4, nce->name, nce->host, nce->type,
			  nce->domain, ZCP_IPv4, &addr, &txt);
			if (res != 0) {
				nce->flags |= NCE_LOOKUPF;
				nclogerr("Zeroconf_lookup() failed [%s] for host '%s [%s]' on interface 0x%x",
				  mos_notice_name(res), nce->name, nce->host, nce->interface);
				goto done;
			}

			taddr = mos_ntoa(&addr, tmp, sizeof (tmp));
			i = kvgeti32(txt, "txtvers", 0);
			if (i == PHIDGET_MDNS_TXTVER) {
				i = kvgeti32(txt, "protocolmajor", 0);
				if (i != PHIDGET_NET_PROTOCOL_MAJOR) {
					ncloginfo("server %s (%s:%d) boardcasting protocol version %d: ignoring server", nce->name,
					  taddr, nce->port, i);
					nce->flags |= NCE_BADVERSION;
					goto done;
				}
			} else {
				ncloginfo("server %s (%s:%d) boardcasting txtvers version %d: ignoring server", nce->name,
				  taddr, nce->port, i);
				nce->flags |= NCE_BADVERSION;
				goto done;
			}

			ncloginfo("network control entry %s: connecting to %s (%s:%d)", nce->name, nce->host, taddr,
			  nce->port);
			...
			if (res != EPHIDGET_OK) {
				nce->failedconn++;
				ncloginfo("failed to connect to MDNS server %s [%s] (%s:%d): %d", nce->name, nce->host, taddr,
				  nce->port, res);
				goto done;
			}
		} else
		...
		{	/* ! NCE_MDNS */
			res = clientConnect(nce->proto, nce->host, nce->port, nce->passwd, PHIDGET_NET_PROTOCOL,
			  ...
			  nce, &nce->nc);
			if (res != EPHIDGET_OK) {
				nclogwarn("failed to connect to server %s (%s:%d):%d", nce->name, nce->host, nce->port, res);
				nce->failedconn++;
				goto done;
			}
		}
