...
wwwServerListener(ZeroconfListenerHandle handle, void *ctx, int added, int interface, Zeroconf_Protocol proto,
  const char *name, const char *host, const char *type, const char *domain) {
	...
	if (serverAdded) {
		res = Zeroconf_lookup(handle, interface, proto, name, host, type, domain, ZCP_IPv4, &addr, &kv);
		if (res != 0) {
		...
		server.stype = PHIDGETSERVER_NAMES[server.type];
		server.handle = NULL;
		server.addr = mos_ntoa(&addr, addrbuf, sizeof(addrbuf));
		server.port = addr.s4.sin_port;
		server.host = host;
		server.flags = 0;
...
deviceServerListener(ZeroconfListenerHandle hdl, void *ctx, int added, int interface, Zeroconf_Protocol proto,
  const char *name, const char *host, const char *type, const char *domain) {
	...
	kv = NULL;
	if (serverAdded)
		res = Zeroconf_lookup(hdl, interface, proto, name, host, type, domain, ZCP_IPv4, &addr, &kv);
	else
		res = Zeroconf_lookup(hdl, interface, proto, name, host, type, domain, ZCP_IPv4, &addr, NULL);
	if (res != 0) {
		netlogerr("Zeroconf_lookup() failed for %s [%s]", name, host);
		return;
	}
	...
	netloginfo("Discovered server '%s' [%s/%s] (interface 0x%x)", name, host,
	  mos_getaddrinfo(&addr, NULL, 0), interface);

	res = PhidgetNet_discoveredServer(hdl, 0, interface, PHIDGETSERVER_DEVICEREMOTE, name, host, type,
	  domain, addr.s4.sin_port);
	if (res != 0)
		netlogerr("failed [%s] to add MDNS server '%s'", mos_notice_name(res), host);
...
readcomplete:
	...
	if (req.nr_flags & NRF_REPLY) {
		...
		if (res != EPHIDGET_OK) {
			netlogerr("reqseq:%d msgtype:%d/%d failed:%s", req.nr_repseq, req.nr_type, req.nr_stype,
			  mos_notice_name(res));
	...
	if (res != EPHIDGET_OK)
		netlogerr("%"PRIphid": msgtype:%d/%d failed:'%s':\n\t%N",
		  nc, req.nr_type, req.nr_stype, mos_notice_name(res), iop);

	/*
	 * We only fail the whole connection for network errors.
	 */
	if (mos_notice_isnetworkrelated(res))
		return (MOS_ERROR(iop, res, "network error on %d/%d", req.nr_type, req.nr_stype));
...
runClientConnection(void *arg) {
	...
	netlogdebug("'%s' thread running for client %"PRIphid"", server->name, server->nc);
...
runAcceptServer(void *arg) {
	...
	netlogdebug(SERVER_FMT " accept thread running", SERVER_ARG);
...
timeout:
	...
	if (flags & PHIDGET_NETWORK_PUBLISHMDNS) {
		...
		if (res == EPHIDGET_OK) {
			...
			netlogerr("failed (%s) to publish '%s' on port %d", mos_notice_name(res),
			  PHIDGET_NETWORK_MDNS_DEVICE, server->port);
		}
	}
#endif
	kvfree(&keys);
...
PhidgetNet_startServer2(PhidgetServerType srvtype, int flags, int af, const char *srvname,
  ...
  handlePhidgetNetConn_t handlenc, handleRequest_t handlereq, PhidgetServerHandle *_server) {
	...
	PhidgetNet_start();
	res = startServer(srvtype, flags, af, srvname, address, port, passwd, initnc, handlenc, handlereq,
	  _server);
	if (res != 0)
		PhidgetNet_stop();
	return (res);
...
PhidgetNet_setPropertyv(const char *key, const char *fmt, va_list va) {
	...
	if (mos_strcmp(key, "keepalive") == 0)
		return (setKeepAlive(val));
	if (mos_strcmp(key, "allowclients") == 0)
		return (setAllowClients(val));
	if (mos_strcmp(key, "allowclient") == 0)
		return (setAllowClient(val));
	if (mos_strcmp(key, "blockclient") == 0)
		return (setBlockClient(val));
	if (mos_strcmp(key, "allowdatagram") == 0)
		return (setAllowDataGram(val));
	if (mos_strcmp(key, "resolveaddrs") == 0)
		return (setResolveAddrs(val));
