...
phidget_init(PhidgetHandle phid, PhidgetStructType type, PhidgetDelete_t _delete) {
	...
	case PHIDGET_CHANNEL:
		mos_tlock_init(phid->__lock, P22LOCK_CHANNELLOCK, P22LOCK_FLAGS);
		mos_tlock_init(phid->__runlock, P22LOCK_CHANNELRUNLOCK, P22LOCK_FLAGS);
		break;
	case PHIDGET_DEVICE:
		mos_tlock_init(phid->__lock, P22LOCK_DEVICELOCK, P22LOCK_FLAGS);
	...
	case PHIDGET_MANAGER:
		mos_tlock_init(phid->__lock, P22LOCK_MANAGERLOCK, P22LOCK_FLAGS);
	...
	case PHIDGET_VIRTUAL_CONNECTION:
		mos_tlock_init(phid->__lock, P22LOCK_CONNECTIONLOCK, P22LOCK_FLAGS);
		mos_tlock_init(phid->__runlock, P22LOCK_CONNECTIONRUNLOCK, P22LOCK_FLAGS);
		break;
	case PHIDGET_NETCONN:
		mos_tlock_init(phid->__lock, P22LOCK_NETCONNLOCK, P22LOCK_FLAGS);
...
PhidgetWait(void *_phid) {
	...
	mos_tlock_wait(&phid->cond, phid->__lock);
}
...
PhidgetTimedWait(void *_phid, uint32_t ms) {
	...
	return (mos_tlock_timedwait(&phid->cond, phid->__lock, (uint64_t)(ms * 1000000)));
}
