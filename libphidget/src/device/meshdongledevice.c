...
PhidgetMeshDongleDevice_initAfterOpen(PhidgetDeviceHandle device) {
	...
	result = PhidgetDevice_sendpacket((PhidgetDeviceHandle)phid, buffer, 5);
...
readInTXBufferCounts(PhidgetMeshDongleDeviceHandle phid) {
	...
	ret = PhidgetDevice_sendpacket((PhidgetDeviceHandle)phid, buffer, 5);
	if (ret != EPHIDGET_OK)
	...
	case PHIDUID_HUB0002:
		...
		while (readPtr < (int)length) {
			...
			if (buffer[readPtr] & MESH_PACKETRETURN_notACK) {
				...
				loginfo("Got an error back from a mesh dongle: 0x%02x, 0x%02x", packetID, response);
			}
			readPtr++;
