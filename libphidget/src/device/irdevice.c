...
static PhidgetReturnCode _Transmit(PhidgetIRDeviceHandle phid, const char *data, PhidgetIR_CodeInfoHandle codeInfo);
static PhidgetReturnCode _TransmitRepeat(PhidgetIRDeviceHandle phid);
static PhidgetReturnCode _TransmitRaw(PhidgetIRDeviceHandle phid, const int *data, int length, int carrierFrequency, double dutyCycle, int gap);
...
PhidgetIRDevice_bridgeInput(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case BP_TRANSMIT:
		...
		return (_Transmit(phid, codeStr, &codeInfo));
	case BP_TRANSMITRAW:
		return (_TransmitRaw(phid, (const int *)getBridgePacketUInt32Array(bp, 0),
...
sendRAWData(PhidgetIRDeviceHandle phid, int *data, int length, int carrier, double dutyCycle, int gap) {
	uint8_t buffer[8] = { 0 };
	int i, j, result;
	...
	if ((result = PhidgetDevice_sendpacket((PhidgetDeviceHandle)phid, buffer, 8)) != EPHIDGET_OK) {
		return result;
	}
...
static PhidgetReturnCode
RawTimeDataToRawData(const int *rawTimeData, int rawTimeDataLength, int *rawData, int *rawDataLength, int *time) {
	int i = 0, j;
...
//returned data must start and end with pulses
static PhidgetReturnCode
codeInfoToRawData(uint8_t *code, PhidgetIR_CodeInfo codeInfo, int *rawData, int *rawDataLength,
...
static PhidgetReturnCode
_Transmit(PhidgetIRDeviceHandle phid, const char *codeStr, PhidgetIR_CodeInfoHandle codeInfo) {
	...
	while (repcount--) {
		...
		dataBufferLength = IR_DATA_ARRAY_SIZE * sizeof(int);
		retval = codeInfoToRawData(datatemp, *codeInfo, dataBuffer, &dataBufferLength, &time, PFALSE);
		...
		retval = sendRAWData(phid, dataBuffer, dataBufferLength, codeInfo->carrierFrequency,
...
static PhidgetReturnCode
_TransmitRaw(PhidgetIRDeviceHandle phid, const int *data, int dataLength, int carrierFrequency,
	...
	if ((retval = RawTimeDataToRawData(data, dataLength, rawData, &rawDataLength, &time)) != EPHIDGET_OK)
		return retval;
	if ((retval = sendRAWData(phid, rawData, rawDataLength, carrierFrequency ? carrierFrequency : 38000, dutyCycle ? dutyCycle : 0.33, gap)) != EPHIDGET_OK)
		return retval;
...
static PhidgetReturnCode
_TransmitRepeat(PhidgetIRDeviceHandle phid) {
	uint8_t datatemp[IR_MAX_CODE_DATA_LENGTH];
	int dataBuffer[IR_DATA_ARRAY_SIZE];
	int dataBufferLength;
	size_t dataSize;
	...
	if (dataSize > 0) {
		...
		dataBufferLength = IR_DATA_ARRAY_SIZE * sizeof(int);
		retval = codeInfoToRawData(datatemp, phid->lastSentCodeInfo, dataBuffer, &dataBufferLength,
			&time, PTRUE);
		if (retval != EPHIDGET_OK)
			return retval;
		...
		//send the data
		retval = sendRAWData(phid, dataBuffer, dataBufferLength, phid->lastSentCodeInfo.carrierFrequency,
