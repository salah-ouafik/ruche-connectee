...
// === Internal Functions === //
static PhidgetReturnCode _sendpacket(PhidgetLEDDeviceHandle phid, int Index);
...
PhidgetLEDDevice_bridgeInput(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case BP_SETSTATE:
		...
		if ((double)getBridgePacketInt32(bp, 0) != phid->LED_Power[ch->index]) {
			...
			return (_sendpacket(phid, LED64_NORMAL_PACKET));
		}
		return (EPHIDGET_OK);
	...
	case BP_SETLEDFORWARDVOLTAGE:
		...
		return (_sendpacket(phid, LED64_CONTROL_PACKET));
	...
	case BP_CLOSERESET:
		if (phid->LED_Power[ch->index] != 0) {
			...
			res = _sendpacket(phid, LED64_NORMAL_PACKET);
			if (res != EPHIDGET_OK)
				return (res);
		}
		switch (phid->phid.deviceInfo.UDD->uid) {
