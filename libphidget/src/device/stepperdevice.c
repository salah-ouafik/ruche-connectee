...
// === Internal Functions === //
static PhidgetReturnCode _sendpacket(PhidgetStepperDeviceHandle phid, int Index);
...
PhidgetStepperDevice_bridgeInput(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHCLASS_STEPPER:
		...
		case BP_SETTARGETPOSITION:
			...
			return (_sendpacket(phid, ch->index | STEPPER_POSITION_PACKET));
		...
		case BP_SETVELOCITYLIMIT:
			...
			case CONTROL_MODE_RUN:
				...
				ret = _sendpacket(phid, ch->index | STEPPER_POSITION_PACKET);
				if (ret != EPHIDGET_OK)
					return (ret);
				break;
			...
			return (_sendpacket(phid, ch->index | STEPPER_VEL_ACCEL_PACKET));
		...
		case BP_CLOSERESET:
			...
			ret = _sendpacket(phid, ch->index | STEPPER_VEL_ACCEL_PACKET);
			if (ret != EPHIDGET_OK)
				return ret;
			// reset the position and target in firmware
			ret = _sendpacket(phid, ch->index | STEPPER_RESET_PACKET);
			if (ret != EPHIDGET_OK)
				return ret;
			ret = _sendpacket(phid, ch->index | STEPPER_POSITION_PACKET);
			if (ret != EPHIDGET_OK)
