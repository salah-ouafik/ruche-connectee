...
// === Internal Functions === //
static PhidgetReturnCode _sendpacket(PhidgetAdvancedServoDeviceHandle phid, int Index);
...
PhidgetAdvancedServoDevice_bridgeInput(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHCLASS_RCSERVO:
		...
		case BP_SETSPEEDRAMPINGSTATE:
			...
			return (_sendpacket(phid, ch->index | ADVSERVO_NORMAL_PACKET));
		case BP_SETENGAGED:
		...
		case BP_SETMINPULSEWIDTH:
			...
			return (_sendpacket(phid, ch->index | ADVSERVO_MINMAX_PACKET));
		...
		case BP_CLOSERESET:
			...
			res = _sendpacket(phid, ch->index | ADVSERVO_MINMAX_PACKET);
			if (res != EPHIDGET_OK)
				return (res);
