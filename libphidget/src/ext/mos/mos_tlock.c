...
#define TLOCKINIT(l)	(ISRWLK(l) ?						\
		mos_rwrlock_init(&((l)->ml_lock.rw)) :				\
		mos_mutex_init(&((l)->ml_lock.mtx)))
#define TLOCKDESTROY(l)	(ISRWLK(l) ?						\
		mos_rwrlock_destroy(&((l)->ml_lock.rw)) :			\
		mos_mutex_destroy(&((l)->ml_lock.mtx)))
#define TLOCKLK(l)	(ISRWLK(l) ?							\
		mos_rwrlock_wrlock(&((l)->ml_lock.rw)) :				\
		mos_mutex_lock(&((l)->ml_lock.mtx)))
#define TLOCKTRYLK(l)	(ISRWLK(l) ? 1 :					\
		mos_mutex_trylock(&((l)->ml_lock.mtx)))
#define TLOCKRLK(l)	(mos_rwrlock_rdlock(&((l)->ml_lock.rw)))
#define TLOCKULK(l)	(ISRWLK(l) ?							\
		mos_rwrlock_unlock(&((l)->ml_lock.rw)) :				\
		mos_mutex_unlock(&((l)->ml_lock.mtx)))
...
_mos_tlock_rlock(mos_tlock_t *this, const char *file, int line, const char *func) {
	...
	/*
	 * If this is a RW lock, then get a read lock; otherwise, just
	 * grab the lock.  We do this to help higher layers more generically
	 * implement RW locks with the option of turning them off completely
